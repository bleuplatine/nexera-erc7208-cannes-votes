...
contract DataIndex is IDataIndex, AccessControl {
    ...
    ///@inheritdoc IDataIndex
    function read(address dobj, DataPoint dp, bytes4 operation, bytes calldata data)
        external
        view
    ...
    ///@inheritdoc IDataIndex
    function write(address dobj, DataPoint dp, bytes4 operation, bytes calldata data)
        external
        onlyApprovedDM(dp)
        returns (bytes memory)
    {
